CREATE DATABASE DataQLKHThanThiet
GO
USE DataQLKHThanThiet
GO
...
BEGIN
	DECLARE @ID NVARCHAR(20)
	IF (SELECT COUNT(MaHang) FROM Hang) = 0
		SET @ID = N'0'
	ELSE
		SELECT @ID = MAX(RIGHT(MaHang, 4)) FROM Hang
		SELECT @ID = CASE
			WHEN @ID >= 0 and @ID < 9 THEN N'RANK000' + CONVERT(NVARCHAR, CONVERT(INT, @ID) + 1)
			WHEN @ID >= 9 THEN N'RANK00' + CONVERT(NVARCHAR, CONVERT(INT, @ID) + 1)
			WHEN @ID >= 99 THEN N'RANK0' + CONVERT(NVARCHAR, CONVERT(INT, @ID) + 1)
			WHEN @ID >= 999 THEN N'RANK' + CONVERT(NVARCHAR, CONVERT(INT, @ID) + 1)
		END
	RETURN @ID
END
GO
...
(
	 ...
	 MaHang NVARCHAR(20) DEFAULT N'RANK0001',
	 TenKH NVARCHAR(100)  NULL,
	 DiaChi NVARCHAR(200) NULL,
	 TichLuy INT DEFAULT 0
	 FOREIGN KEY (MaHang) REFERENCES dbo.Hang
)
GO
...
GO
CREATE FUNCTION GetTichLuy(@sdtKH NVARCHAR(20))
RETURNS INT
BEGIN
	DECLARE @tichLuy INT =(SELECT SUM(TongTien) " +
                "FROM DonHang WHERE DonHang.SDTKH=@sdtKH AND TrangThai=N'Đã hoàn thành')
	IF @tichLuy IS NULL
		RETURN 0
	RETURN @tichLuy
END
...
GO
INSERT INTO Hang VALUES(N'RANK0001',N'Vô hạng',0,0)
GO